...
def _safe_div(numerator, denominator):
    ...
    The closed-form Love/shape-number formulas below (Damour & Nagar 2009)
    are rational functions in (compactness c, y) whose denominator
    polynomials have genuine algebraic roots at certain (c, y) points --
    dividing through one produces +-inf (the numerator is generically
    nonzero there; IEEE division only gives nan for the 0/0 case), not nan.
    An infinite Love/shape number is a worse silent-failure mode than an
    explicit nan: it isn't caught as consistently by isfinite-style
...
class TOV(object):
    ...
    * https://lscsoft.docs.ligo.org/bilby/_modules/bilby/gw/eos/tov_solver.html
    ...
    def solve(self,pc):
        ...
        self.C = C
        self.sol = sol
        # The closed-form Love number polynomials below are only valid
        # for a physical compactness 0 < C < 0.5 (Schwarzschild limit);
        # non-converged solves (e.g. the lowest-pc end of a tov_series,
        # see utils.Target) can return M<=0/R<=0, hence C<=0, for which
        # these polynomials hit exact-zero denominators (divide-by-zero
        # RuntimeWarning) rather than producing a meaningful answer.
        # Skip straight to NaN for those instead of evaluating them.
        physical_C = 0 < C < 0.5
        if len(self.leven) != 0:
            k, h = {}, {}
            for l in self.leven:
    ...
    def __compute_Love_odd(self,ell,c,y):
        ...
        * the ratio y = R Psi(R)'/Psi(R)
        ...
        if ell == 2:
            ...
            factor =  - c ** (2 * ell + 1)
            j = factor * _safe_div(dPsiP - c * y * PsiP, dPsiQ - c * y * PsiQ)
        return j
    ...
    def __compute_shape(self,ell,c,y):
        ...
        * the ratio y = R H(R)'/H(R)
        ...
        #     h = -64*c9*nh/dh
            ...
            factor = c**(ell+1)*Pl2 * (1-_safe_div(dPl2/Pl2-c*y, dQl2/Ql2-c*y))
