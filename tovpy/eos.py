...
class EOSTabular(object):
    ...
    def __interpolate(self,kind='cubic'):
        ...
        nTab = self.table[:, 3] * self.uts.cgs_to_geom('radius')**-3
        while np.any(msk := np.diff(pTab) <= 0):
            istart = np.where(msk)[0].min()
            pval = pTab[istart]
            msk = pTab[istart+1:] > pval
            if not np.any(msk):
                iend = len(pTab)
            else:
                iend = np.where(msk)[0].min() + istart + 1
            print(f"Warning: {self.name} removing entries {istart+1}/{len(pTab)} to {iend-1}/{len(pTab)} with non-monotonic pressure values.")
            del_slice = slice(istart+1, iend)
            nTab = np.delete(nTab, del_slice)
            eTab = np.delete(eTab, del_slice)
            pTab = np.delete(pTab, del_slice)
    ...
    def Pressure_Of_EnergyDensity(self,e):
        ...
        # p[ids_gt_min] = np.exp(self.interp_logPressure_from_logEnergyDensity(np.log(e[ids_gt_min])))
        # if p.size == 1:
        #     return p[0]
        # return p
